...
int main(int argc, char *argv[])
{
    ...
    if (connect(clifd, (struct sockaddr *) &srvinfo, sizeof(srvinfo)) < 0)
    {
    	printf("A kapcsolodas sikeretelen.\n");
    	return 1;
    }
    ...
    while (1)
    {
    	...
    	// új áru
    	{
    		...
    		if (write(clifd, &p, sizeof(p)) < 0)
    		{
    			printf("A kuldes sikertelen.\n");
    			return 1;
    		}

    		read(clifd, &valasz_st, 1);
    		printf("valasz: %c\n", valasz_st);
    		if (valasz_st == 'o')
    		{
    	...
    	else if (n == 1 && !strcmp(parancs, "list"))
    	{
    		...
    		read(clifd, &valasz_raktar, MAX_ARUK * sizeof(struct arucikk));
